...
# Make an dataframe of age and growth data just for females
...
# JMS: limit size of the object first, then do operations on it
tempData = tempData[tempData$year > 2007 & tempData$year < 2014 ,]

# Summarize the temperature by day across years
mu = ddply(tempData, .(day, year), summarize, val = mean(val, na.rm =
                                                           TRUE))
...
load('datasets/tempD.rda')
# Summarize the temperature by day across years
hmu = ddply(tempD, .(day, year), summarize, val = mean(val, na.rm =
                                                         TRUE))
